...
multiple_length during each epoch \n
Model is mBERT \n
"""
train_settings = "small_sample_shuffled_sentences_multiple_length"
...
with open(f'../logs/train-{train_settings}_{num_epochs}.txt', 'w') as loss_log:
    ...
    for epoch in range(num_epochs):
        ...
        #percent_and_max_len = ((0.003, 18), (0.004, 10), (0.003, 20), (0.003, 40), (0.001, 70))
        percent_and_max_len = ((0.1, 10), (0.1, 20), (0.1, 40), (0.1, 70), (0.1, 140))


        training_data_0 = pd.read_csv(f"../data/daily_dialog/train/train_4-8_words.txt").sample(frac=percent_and_max_len[0][0])
        training_data_1 = pd.read_csv(f"../data/daily_dialog/train/train_4-16_words.txt").sample(frac=percent_and_max_len[1][0])
        training_data_2 = pd.read_csv(f"../data/daily_dialog/train/train_4-32_words.txt").sample(frac=percent_and_max_len[2][0])
        training_data_3 = pd.read_csv(f"../data/daily_dialog/train/train_4-64_words.txt").sample(frac=percent_and_max_len[3][0])
        training_data_4 = pd.read_csv(f"../data/daily_dialog/train/train_4-128_words.txt").sample(frac=percent_and_max_len[4][0])
        ...
        #chunk = 0.1% of whole dataset
        chunk_size= num_rows = sum(1 for _ in open('../data/daily_dialog/train/train_4-8_words.txt')) // 1000

        for setting in percent_and_max_len:
            ...
            for chunk_num, chunk in enumerate(data_iterator):
                ...
                df = chunk.sample(frac=1.) #shuffle
                #df = chunk #no shuffle
                ...
                try:
                    inputs = [tokenizer.encode_plus(add_mask(text), add_special_tokens=False, truncation=True,
                                                    max_length=setting[1], padding='max_length') for text in
                              df["input"] if isinstance(text, str)]
