...
#settings = ((50,15,10,5,5),(50,20,10,5,5),(40,20,10,5,5),(20,20,20,20,20),(40,20,10,5,5))
...
###     Evaluate every 25 batches     ###
        if (batch + 1) % 25 == 0:  # Using 10 as the modulus since it's the LCM of 5 and 10
                ...
                print("Validating")
                performance = f"{bert_accuracy(input_model=model, input_tokenizer=tokenizer, percentage=0.025, test_dataset=False)}\n"
                logs += f"Epoch: {epoch}  Batch: {batch}: {performance}"
                print(performance)
